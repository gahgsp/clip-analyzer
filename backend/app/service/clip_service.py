...
from app.core.config import ClipServiceConfiguration


class ClipService:
    def __init__(self, configuration: ClipServiceConfiguration):
        self.frames_dir = configuration.frames_dir
        self.percentages = configuration.percentages
        self.max_duration = configuration.max_duration
        self.ytdlp_options = configuration.ytdlp_options
    ...
    def process_clip(self, url: str) -> ProcessedClip:
        ...
        if duration > self.max_duration:
            raise ClipTooLongError(
                duration=duration, max_duration=self.max_duration)
    ...
    def _resolve_stream_info(self, url: str) -> Tuple[str, float, str]:
        ...
        try:
            with yt_dlp.YoutubeDL(params=self.ytdlp_options) as ydl:
